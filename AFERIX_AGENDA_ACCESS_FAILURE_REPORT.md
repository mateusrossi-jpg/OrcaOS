...
Como o `App.tsx` não envolvia o `FieldWorkspace` em um `RuntimeErrorBoundary` (apenas a tela de Propostas possuía essa proteção), qualquer erro de renderização no componente de Agenda resultava na queda de toda a árvore de componentes, exibindo uma tela preta/vazia acima do Dock de navegação.
...
8. **Existe perfil ocultando a Agenda?** 
   - Não, o perfil SOLO agora possui acesso direto via Dock.
