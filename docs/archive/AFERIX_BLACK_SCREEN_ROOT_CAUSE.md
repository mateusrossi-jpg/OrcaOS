...
As cascas de perfil (ex: `SoloShell` em `RoleShells.tsx`) apenas estruturam o layout (Menu inferior e Container principal). Toda a interface gráfica, incluindo a cor de fundo `bg-[var(--bg-primary)]` definida no `ScreenContainer` e o `AppHeader`, é injetada pelos componentes "filhos" (as telas). 
Quando uma rota falha em dar match:
- O `children` passado para a Shell se torna vazio (`null`).
- Como consequência, a página fica sem background explícito e sem cabeçalho, resultando num vazio absoluto (a tela preta) acima do menu de navegação.
