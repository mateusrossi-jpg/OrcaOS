...
- **Botões de Fechar Modal (X):** Tradicionalmente pequenos e no topo da tela, dificultam o fechamento com o ergonomia.

## 5. Quais CTAs estão longe da região natural do ergonomia?
**Excelente (com exceções focais)**
- O sistema brilha no uso da `StickyActionBar` (como no `BudgetForm`) e no `TacticalActionBar` global, que mantêm a navegação no alcance do ergonomia.
