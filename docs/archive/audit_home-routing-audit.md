...
3.  **Vignette & AppShell Clashing:** O `AppShell.tsx` real envolve as telas em containers com paddings e bordas adicionais, e possui seu próprio Menu inferior de 3 ações. Isso espreme e quebra o layout figma original que esperava um viewport de container livre.
...
Ajustar `SurfaceCard` e `BottomMenu` em `/src/ui/system/AferixComponents.tsx` para usarem os exatos estilos inline do Figma caso o Tailwind falhe ou o navegador recuse a renderização do gradiente.
