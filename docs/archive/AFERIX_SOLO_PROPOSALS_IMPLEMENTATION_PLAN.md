...
## 1. Arquivos Afetados
* `src/features/workspace/components/RoleShells.tsx`: Definição das abas do Menu para o perfil SOLO.
...
* `Sistema de Navegação`: Componente visual que renderiza as abas inferiores.
...
* **Densidade do Menu:** Ao adicionar a 6ª aba (Meu Negócio, Agenda, Propostas, Financeiro, Clientes, Menu), o Menu pode apresentar compressão visual em dispositivos com telas muito pequenas (iPhone SE, etc.).
* **Sobreposição de Ações:** Garantir que o botão de "Novo Orçamento" na Home e a nova aba no menu não gerem confusão de fluxo, mas sim caminhos complementares (Criação vs. Gestão).
