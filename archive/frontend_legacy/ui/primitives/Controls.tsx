...
export const SearchInput = memo(({ className, onChange, onResultSelect, ...props }: SearchInputProps) => {
  ...
  return (
    <div ref={containerRef} className="relative w-full z-40">
      <div className={cn(
        "relative flex items-center h-[56px] rounded-[20px] bg-white/[0.04] border border-white/5 px-5 focus-within:border-[var(--accent-blue)]/30 focus-within:bg-white/[0.06] transition-all duration-300 cubic-bezier(0.16, 1, 0.3, 1) shadow-sm group",
        className
      )}>
      ...
      {showOverlay && (
        <div className="absolute top-[64px] left-0 right-0 bg-[#0A0A0A]/95 backdrop-blur-[40px] border border-white/[0.08] rounded-[28px] shadow-[0_30px_90px_rgba(0,0,0,0.9)] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-400 ease-[cubic-bezier(0.16,1,0.3,1)]">
           {!query ? (
             <div className="p-6 flex flex-col gap-8">
                {intelligence?.recentClients && intelligence.recentClients.length > 0 && (
                  <div className="flex flex-col gap-4">
                     <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] ml-1">Clientes Recentes</span>
                     <div className="flex gap-3 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                        {intelligence.recentClients.map(c => (
                          <button 
                            key={c.id}
                            onClick={() => {
                              onResultSelect?.({ type: 'client', ...c });
                              setIsFocused(false);
                            }}
                            className="flex-none h-14 px-6 rounded-2xl bg-white/[0.04] border border-white/5 flex items-center gap-3 active:scale-95 transition-all whitespace-nowrap"
                          >
                             <User size={16} className="text-[var(--accent-gold)]" />
