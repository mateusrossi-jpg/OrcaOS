...
const newRender = `  const isExecuting = nextActionOS?.status === 'in-progress' || nextActionOS?.status === 'en_route';
      ...
      {nextActionOS ? (
         <div className="bg-[#0A84FF]/10 border border-[#0A84FF]/40 rounded-[16px] p-4 flex flex-col gap-3.5 relative overflow-hidden group shadow-[0_8px_30px_rgba(10,132,255,0.15)]">
            ...
            <div className="flex items-center gap-2 mt-2">
               <button 
                 ...
                 className="flex-1 h-11 bg-transparent border border-white/[0.15] hover:border-white/[0.3] text-white font-bold text-[10px] uppercase tracking-widest rounded-lg flex items-center justify-center transition-all duration-200 active:scale-[0.98]"
               >
                  VER DETALHES
               </button>
               <button 
                 onClick={(e) => { e.stopPropagation(); onNavigate({ tab: 'operations', workOrderId: nextActionOS.id }); }}
                 className="flex-1 h-11 bg-[#0A84FF] hover:bg-[#0A84FF]/90 text-white font-bold text-[10px] uppercase tracking-widest rounded-lg flex items-center justify-center transition-all duration-200 shadow-md active:scale-[0.98]"
               >
                  INICIAR ROTA
               </button>
            </div>
         ...
         <div className="bg-white/[0.02] border border-white/[0.04] rounded-[16px] p-4 flex flex-col items-start gap-4 transition-all">
            ...
            <button
              ...
              className="px-6 h-11 bg-[#0A84FF] hover:bg-[#0A84FF]/90 text-white font-black text-[10px] uppercase tracking-widest rounded-lg flex items-center justify-center gap-2 transition-all duration-200 shadow-sm active:scale-[0.98]"
            >
      ...
      {topOpportunity ? (
        ...
        >
          ...
          <button 
            ...
            className="w-full h-11 bg-[var(--accent-gold)] hover:bg-[var(--accent-gold)]/90 text-black font-bold text-[10px] uppercase tracking-widest rounded-lg flex items-center justify-center transition-all duration-200 shadow-sm active:scale-[0.98] mt-1"
          >
        ...
        <div className="bg-white/[0.02] border border-white/[0.04] rounded-[16px] p-4 flex flex-col items-start gap-4 transition-all">
           ...
           <button
             onClick={() => onNavigate('new-budget')}
             className="px-6 h-11 bg-[var(--accent-gold)] hover:bg-[var(--accent-gold)]/90 text-black font-black text-[10px] uppercase tracking-widest rounded-lg flex items-center justify-center gap-2 transition-all duration-200 shadow-sm active:scale-[0.98]"
           >
  ...
  // 3. MONEY BLOCK (LEVE PROFUNDIDADE E COMPACTO)
  const renderMoney = () => (
    <div className="flex flex-col gap-1.5">
      ...
      <div 
        ...
        className="bg-[#28A745]/[0.02] border border-[#28A745]/[0.08] p-3 rounded-[16px] flex flex-col gap-2 group active:scale-[0.98] transition-all duration-200 cursor-pointer"
      >
        <div className="flex justify-between items-center">
          <div className="flex flex-col gap-1">
            <span className="text-[9px] font-black text-[#28A745] uppercase tracking-[0.2em]">A RECEBER</span>
            <span className="text-[17px] font-mono font-bold text-[#28A745] leading-none">
              {formatCurrencyBRL(stats.toCollect)}
            </span>
          </div>
          <span className="text-[10px] font-medium text-white/40 uppercase tracking-wider text-right leading-tight">
      ...
      <div className="px-5 pt-4 pb-[80px] flex flex-col relative z-10 max-w-md mx-auto w-full">
        ...
        <div className="flex flex-col mt-0.5 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-4">
            <div className="bg-white/[0.04] border border-white/[0.08] px-2.5 py-1 rounded-md flex items-center justify-center">
            ...
            <div className="bg-white/[0.04] border border-white/[0.08] px-2.5 py-1 rounded-md flex items-center justify-center">
              <span className="text-[10px] font-bold text-[#30D158]/90">
                {formatCurrencyBRL(stats.toCollect)}
              </span>
            </div>
          </div>
        ...
        <div className="mt-6 pt-0">
          <div className="w-full relative flex items-center group">
            ...
            <input 
              ...
              className="w-full h-11 pr-10 bg-white/[0.02] border border-white/[0.05] rounded-[12px] text-white text-[12px] placeholder:text-white/20 outline-none focus:border-white/20 focus:bg-white/[0.04] transition-all duration-200"
...
function GridTool({ icon, title, sub, onClick }: any) {
  return (
    <div 
      onClick={onClick}
      className="bg-white/[0.02] border border-white/[0.04] hover:bg-white/[0.06] active:scale-[0.98] transition-all duration-200 rounded-[12px] p-3 flex flex-col cursor-pointer group"
    >
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
        ...
        <div className="mt-1 pt-2 border-t border-white/[0.03] flex justify-end items-center text-[9px] font-bold text-white/30 uppercase tracking-widest group-hover:text-white/60 transition-colors duration-200">
