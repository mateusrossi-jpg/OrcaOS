...
function groupLinesByEnvironment(lines: GuidedLine[]): EnvironmentGroup[] {
  const groups = new Map<string, GuidedLine[]>();
  lines.forEach((line) => {
    const environment = line.environment.trim() || 'Sem ambiente';
    groups.set(environment, [...(groups.get(environment) ?? []), line]);
  });

  return Array.from(groups.entries()).map(([environment, groupLines]) => {
    const subtotal = groupLines.reduce((sum, line) => sum + lineTotal(line), 0);
    const materialSubtotal = groupLines.filter((line) => line.itemType === 'material').reduce((sum, line) => sum + lineTotal(line), 0);
    const serviceSubtotal = groupLines.filter((line) => line.itemType === 'service').reduce((sum, line) => sum + lineTotal(line), 0);
...
export function GuidedBudgetCart({ onSendToBudget, mode = 'all' }: GuidedBudgetCartProps) {
  ...
  const environmentGroups = useMemo(() => groupLinesByEnvironment(lines), [lines]);
  ...
  return (
    <section className="guided-cart-panel">
      ...
      {showCatalog && (
        <div className="guided-manual-block-card">
          <div><strong>Mão de obra guiada</strong><small>Clique para somar serviços. Ajuste quantidade e valor antes de adicionar.</small></div>
      ...
      <div className="guided-cart-summary grouped-summary">
        ...
        {environmentGroups.length === 0 ? <small>Nenhum item adicionado ainda.</small> : (
          <div className="environment-summary-list">
            {environmentGroups.map((group) => (
              <article className="environment-summary-card" key={group.environment}>
                <header>
                  <div>
                ...
                <div className="environment-line-preview">
                  {group.lines.slice(0, 5).map((line) => <span key={line.id}>{line.quantity}× {line.description}</span>)}
      ...
      {environmentGroups.length > 0 && (
        <div className="environment-grouped-editor">
          {environmentGroups.map((group) => (
            <section className="environment-editor-group" key={group.environment}>
              <header>
                <div>
                  <span>Ambiente</span>
                  <strong>{group.environment}</strong>
              ...
              <div className="parts-result-list">
                {group.lines.map((line) => (
                  <article className="part-result-card active" key={line.id}>
                    <div className="part-result-main"><span>{kindLabel(line.kind)} · {line.itemType === 'material' ? 'Material' : 'Serviço'}</span><strong>{line.description}</strong><small>{line.brand ? `${line.brand}${line.model ? ` · ${line.model}` : ''}` : line.note}</small></div>
