...
function emptyResult(): BudgetResult {
  return { error: null, summary: '', details: [], orientation: '', cards: [] };
}

function result(summary: string, cards: ResultCardData[], details: string[], orientation: string): BudgetResult {
...
export function TechnicalBudgetHumanWorkspace({ onCaptureCalculation }: Props) {
  ...
  const calculated = useMemo<BudgetResult>(() => {
    ...
    try {
      if (activeRule.mode === 'labor') {
        ...
        return result(
          `Mão de obra: ${money(total)}`,
          [
            { label: 'Subtotal', value: money(subtotal), helper: `${round(quantity)} × ${money(unitValue)}` },
            { label: 'Deslocamento', value: money(travel), helper: 'custo adicional' },
            { label: 'Total', value: money(total), helper: 'valor sugerido' },
          ],
          [`Quantidade: ${round(quantity)}`, `Valor unitário: ${money(unitValue)}`, `Subtotal: ${money(subtotal)}`, `Deslocamento: ${money(travel)}`, `Total: ${money(total)}`],
          'Use para serviços repetitivos por ponto, peça, luminária ou tomada. Ajuste dificuldade, altura, deslocamento e urgência antes de fechar.',
        );
      }
      ...
      if (activeRule.mode === 'final-price') {
        ...
        return result(
          `Preço final: ${money(total)}`,
          [
            { label: 'Base', value: money(base), helper: 'material + mão de obra + deslocamento' },
            { label: 'Margem/impostos', value: money(profitValue + taxValue), helper: `${round(profitPercent)}% + ${round(taxPercent)}%` },
            { label: 'Total', value: money(total), helper: discountValue > 0 ? `desconto ${money(discountValue)}` : 'sem desconto' },
          ],
          [`Material: ${money(material)}`, `Mão de obra: ${money(labor)}`, `Deslocamento: ${money(travel)}`, `Margem: ${money(profitValue)}`, `Impostos: ${money(taxValue)}`, `Desconto: ${money(discountValue)}`, `Total: ${money(total)}`],
          'Use como fechamento comercial. Em proposta real, separe material, mão de obra, garantia, prazo, forma de pagamento e validade.',
        );
      }
      ...
      if (activeRule.mode === 'daily') {
        ...
        return result(
          `Diária: ${money(total)}`,
          [
            { label: 'Profissional', value: money(labor), helper: `${round(days)} dia(s)` },
            { label: 'Ajudante', value: money(helper), helper: helperDailyValue > 0 ? `${money(helperDailyValue)}/dia` : 'não informado' },
            { label: 'Total', value: money(total), helper: travel > 0 ? `inclui ${money(travel)} deslocamento` : 'sem deslocamento' },
          ],
          [`Dias: ${round(days)}`, `Diária profissional: ${money(dailyValue)}`, `Ajudante/dia: ${money(helperDailyValue)}`, `Deslocamento: ${money(travel)}`, `Total: ${money(total)}`],
          'Use quando o serviço não cabe bem por ponto ou unidade. Defina claramente o que está incluso na diária.',
        );
      }
      ...
      if (activeRule.mode === 'hourly') {
        ...
        const total = subtotal + travel;
        return result(
          `Hora técnica: ${money(total)}`,
          [
            { label: 'Horas', value: `${round(hours)} h`, helper: `${money(hourlyValue)}/h` },
            { label: 'Subtotal', value: money(subtotal), helper: 'sem deslocamento' },
            { label: 'Total', value: money(total), helper: travel > 0 ? `inclui ${money(travel)} deslocamento` : 'sem deslocamento' },
          ],
          [`Horas: ${round(hours)} h`, `Valor hora: ${money(hourlyValue)}`, `Subtotal: ${money(subtotal)}`, `Deslocamento: ${money(travel)}`, `Total: ${money(total)}`],
          'Use para diagnóstico, visita técnica, programação, manutenção ou serviços de duração variável.',
        );
      }
      ...
      if (activeRule.mode === 'installments') {
        ...
        return result(
          `${round(installments)}× de ${money(installmentValue)}`,
          [
            { label: 'Total original', value: money(total), helper: 'sem acréscimo' },
            { label: 'Total parcelado', value: money(adjustedTotal), helper: `${round(interestPercent)}% de acréscimo` },
            { label: 'Parcela', value: money(installmentValue), helper: `${round(installments)} parcela(s)` },
          ],
          [`Total original: ${money(total)}`, `Parcelas: ${round(installments)}`, `Acréscimo: ${round(interestPercent)}%`, `Total parcelado: ${money(adjustedTotal)}`, `Valor da parcela: ${money(installmentValue)}`],
          'Use para apresentar condição de pagamento. Confira taxas reais da maquininha, banco ou plataforma antes de prometer parcelamento.',
        );
      }
      ...
      return result(
        `Entrada: ${money(upfrontValue)}`,
        [
          { label: 'Entrada', value: money(upfrontValue), helper: `${round(upfrontPercent)}% do total` },
          { label: 'Saldo', value: money(remaining), helper: 'restante a receber' },
          { label: 'Total', value: money(total), helper: 'valor da proposta' },
        ],
        [`Total: ${money(total)}`, `Percentual de entrada: ${round(upfrontPercent)}%`, `Entrada: ${money(upfrontValue)}`, `Saldo: ${money(remaining)}`],
        'Use para sinal de material, reserva de agenda ou início de serviço. Combine por escrito quando o saldo será pago.',
      );
    } catch (error) {
  ...
  function includeResult(destination: CalculationDestination) {
    ...
    const capture: CalculationCapture = {
      id: createId('technical-budget-calc'),
      module: 'orcamentoTecnico',
      moduleLabel: 'Orçamento técnico',
  ...
  function openCalculator(mode: BudgetMode) {
    setActiveMode(mode);
    setAddedMessage(null);
    setShowAdvanced(false);
  }
  ...
  return (
    <div className="general-calculator-workspace">
      <div className="general-plan-banner">
        <div>
          <strong>Orçamento técnico V1</strong>
          <span>Cálculos comerciais simples para serviço, preço final, diária, hora técnica e pagamento.</span>
        </div>
      ...
      <div className="general-picker-list">
        {rules.map((rule) => (
          <button className="general-picker-card" key={rule.mode} type="button" onClick={() => openCalculator(rule.mode)}>
            <span className="app-icon tone-orange">{rule.icon}</span>
            <span>
