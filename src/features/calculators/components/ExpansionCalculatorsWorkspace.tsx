...
const rules: Rule[] = [
  ...
  { mode: 'tax-fee-gross-up', module: 'financeiroAvancado', label: 'Receber líquido desejado', description: 'Preço bruto para sobrar um valor líquido após taxas.', plan: 'pro', fields: [{ key: 'desiredNet', label: 'Quero receber líquido', suffix: 'R$' }, { key: 'tax', label: 'Imposto/taxa', suffix: '%' }, { key: 'cardFee', label: 'Taxa cartão/plataforma', suffix: '%' }], compute: ({ n, nn }) => { const net = n('desiredNet', 'valor líquido'); const totalRate = (nn('tax', 'taxas') + nn('cardFee', 'taxa de cartão')) / 100; const gross = net / (1 - totalRate); const fees = gross - net; return result(`Cobrar: ${money(gross)}`, [{ label: 'Valor bruto', value: money(gross) }, { label: 'Taxas estimadas', value: money(fees) }, { label: 'Líquido', value: money(net) }], [`Cobrar ${money(gross)} para receber ${money(net)} líquido`], 'Use quando você sabe quanto precisa sobrar depois de taxa, imposto ou plataforma.', ['Preço bruto = líquido desejado ÷ (1 - taxa total).']); } },
  { mode: 'card-fee-net', module: 'financeiroAvancado', label: 'Quanto sobra no cartão', description: 'Valor líquido após taxa de cartão e imposto.', plan: 'pro', fields: [{ key: 'price', label: 'Valor cobrado', suffix: 'R$' }, { key: 'cardFee', label: 'Taxa cartão', suffix: '%' }, { key: 'tax', label: 'Imposto/taxa', suffix: '%' }], compute: ({ n, nn }) => { const price = n('price', 'valor'); const card = price * nn('cardFee', 'cartão') / 100; const tax = price * nn('tax', 'imposto') / 100; const net = price - card - tax; return result(`Líquido estimado: ${money(net)}`, [{ label: 'Cartão', value: money(card) }, { label: 'Imposto/taxa', value: money(tax) }, { label: 'Sobra', value: money(net) }], [`Líquido: ${money(net)}`], 'Útil para decidir se absorve taxa ou repassa ao cliente.', ['Líquido = valor cobrado - taxa cartão - imposto/taxa.']); } },
  { mode: 'cash-vs-installment', module: 'financeiroAvancado', label: 'À vista vs parcelado', description: 'Compare desconto à vista com parcelamento.', plan: 'pro', fields: [{ key: 'price', label: 'Preço base', suffix: 'R$' }, { key: 'discount', label: 'Desconto à vista', suffix: '%' }, { key: 'cardFee', label: 'Taxa do parcelado', suffix: '%' }, { key: 'installments', label: 'Parcelas' }], compute: ({ n, nn }) => { const price = n('price', 'preço'); const cash = price * (1 - nn('discount', 'desconto') / 100); const installmentFee = price * nn('cardFee', 'taxa') / 100; const installmentNet = price - installmentFee; return result(`À vista: ${money(cash)} · Parcelado líquido: ${money(installmentNet)}`, [{ label: 'À vista', value: money(cash) }, { label: 'Parcelado líquido', value: money(installmentNet) }, { label: 'Parcela', value: money(price / n('installments', 'parcelas')) }], ['Comparação comercial gerada'], 'Compare o que realmente sobra, não apenas o valor anunciado ao cliente.', ['À vista = preço × (1 - desconto)', 'Líquido parcelado = preço - taxa.']); } },
  { mode: 'break-even-services', module: 'financeiroAvancado', label: 'Serviços para empatar', description: 'Quantos serviços pagam custos fixos e renda alvo.', plan: 'pro', fields: [{ key: 'fixedCost', label: 'Custos fixos mensais', suffix: 'R$' }, { key: 'desiredIncome', label: 'Renda desejada', suffix: 'R$' }, { key: 'averageProfit', label: 'Lucro médio por serviço', suffix: 'R$' }], compute: ({ n }) => { const target = n('fixedCost', 'custos fixos') + n('desiredIncome', 'renda'); const services = Math.ceil(target / n('averageProfit', 'lucro médio')); return result(`${services} serviço(s) para atingir a meta`, [{ label: 'Meta mensal', value: money(target) }, { label: 'Lucro médio', value: money(n('averageProfit', 'lucro médio')) }, { label: 'Serviços', value: `${services}` }], [`Meta mensal: ${money(target)}`], 'Ajuda a transformar custo fixo e renda desejada em volume mínimo de serviços.', ['Serviços = (custos fixos + renda desejada) ÷ lucro médio por serviço.']); } },
  { mode: 'monthly-goal', module: 'financeiroAvancado', label: 'Meta mensal por ticket', description: 'Faturamento e atendimentos necessários no mês.', plan: 'pro', fields: [{ key: 'desiredIncome', label: 'Meta de renda/lucro', suffix: 'R$' }, { key: 'fixedCost', label: 'Custos fixos', suffix: 'R$' }, { key: 'averageTicket', label: 'Ticket médio', suffix: 'R$' }, { key: 'margin', label: 'Margem líquida média', suffix: '%' }], compute: ({ n }) => { const neededProfit = n('desiredIncome', 'meta') + n('fixedCost', 'custos'); const profitPerTicket = n('averageTicket', 'ticket') * n('margin', 'margem') / 100; const jobs = Math.ceil(neededProfit / profitPerTicket); const revenue = jobs * n('averageTicket', 'ticket'); return result(`${jobs} atendimento(s) no mês`, [{ label: 'Faturamento alvo', value: money(revenue) }, { label: 'Lucro/meta', value: money(neededProfit) }, { label: 'Atendimentos', value: `${jobs}` }], [`Faturamento alvo: ${money(revenue)}`], 'Use para saber se a agenda e o preço médio sustentam sua meta.', ['Atendimentos = meta de lucro ÷ (ticket médio × margem líquida).']); } },
  { mode: 'material-resale-price', module: 'financeiroAvancado', label: 'Revenda de material', description: 'Preço de material com margem, taxa e desconto.', plan: 'pro', fields: [{ key: 'materialCost', label: 'Custo do material', suffix: 'R$' }, { key: 'margin', label: 'Margem desejada', suffix: '%' }, { key: 'tax', label: 'Taxa/imposto', suffix: '%' }, { key: 'discount', label: 'Desconto possível', suffix: '%' }], compute: ({ n, nn }) => { const price = calculateSalePriceByTargetMargin({ cost: n('materialCost', 'material'), marginPercent: n('margin', 'margem'), taxPercent: nn('tax', 'taxa'), plannedDiscountPercent: nn('discount', 'desconto') }); return result(`Material por: ${money(price.suggestedPrice)}`, [{ label: 'Custo', value: money(n('materialCost', 'material')) }, { label: 'Venda', value: money(price.suggestedPrice) }, { label: 'Lucro', value: money(price.profit) }], [`Preço de venda do material: ${money(price.suggestedPrice)}`], 'Útil quando o profissional compra, garante ou entrega material ao cliente.', ['Preço = custo do material ÷ (1 - margem), com taxa e desconto planejado.']); } },
  { mode: 'warranty-reserve', module: 'financeiroAvancado', label: 'Reserva para garantia', description: 'Separe parte do orçamento para retorno, garantia ou retrabalho.', plan: 'pro', fields: [{ key: 'price', label: 'Valor do serviço', suffix: 'R$' }, { key: 'warrantyPercent', label: 'Reserva', suffix: '%' }], compute: ({ n }) => { const reserve = n('price', 'valor') * n('warrantyPercent', 'reserva') / 100; return result(`Reserva sugerida: ${money(reserve)}`, [{ label: 'Reserva', value: money(reserve) }, { label: 'Disponível', value: money(n('price', 'valor') - reserve) }], [`Reserva: ${money(reserve)}`], 'Ajuda a não tratar garantia/retrabalho como surpresa financeira.', ['Reserva = valor do serviço × percentual de reserva.']); } },
  { mode: 'emergency-price', module: 'financeiroAvancado', label: 'Preço de urgência', description: 'Acréscimo para atendimento urgente, noite, fim de semana ou risco.', plan: 'pro', fields: [{ key: 'price', label: 'Preço normal', suffix: 'R$' }, { key: 'emergencyPercent', label: 'Acréscimo de urgência', suffix: '%' }, { key: 'travelCost', label: 'Deslocamento extra', suffix: 'R$' }], compute: ({ n, nn }) => { const surcharge = n('price', 'preço') * n('emergencyPercent', 'urgência') / 100; const final = n('price', 'preço') + surcharge + nn('travelCost', 'deslocamento'); return result(`Preço de urgência: ${money(final)}`, [{ label: 'Acréscimo', value: money(surcharge) }, { label: 'Deslocamento', value: money(nn('travelCost', 'deslocamento')) }, { label: 'Total', value: money(final) }], [`Preço urgente: ${money(final)}`], 'Use para atendimento fora do padrão sem misturar urgência com orçamento comum.', ['Total = preço normal + acréscimo de urgência + deslocamento extra.']); } },
];
      ...
      {lockedRule && (
        <div className="general-calculator-overlay" role="dialog" aria-modal="true" aria-label={proFeatureTitle(lockedRule.plan)}>
          ...
          <section className="general-overlay-panel general-upgrade-panel">
            <header className="general-overlay-header"><button type="button" onClick={() => setLockedRule(null)}>‹</button><div><span>{moduleLabel(lockedRule.module)}</span><h2>{proFeatureTitle(lockedRule.plan)}</h2><p>{lockedRule.label} faz parte dos cálculos profissionais do OrçaOS.</p></div><em>PRO</em></header>
