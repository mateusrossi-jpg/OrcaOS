...
const commercialSections = [1.5, 2.5, 4, 6, 10, 16, 25, 35, 50, 70, 95, 120];
const commercialBreakers = [6, 10, 16, 20, 25, 32, 40, 50, 63, 70, 80, 100, 125];
const awgTable = [
...
function voltageDrop(current: number, distance: number, section: number, voltage: number, phase: string, conductor: string) {
  ...
  const volts = multiplier * rho * distance * current / section;
  return { volts, percent: volts / voltage * 100 };
}

function suggestSectionByCurrent(current: number): number {
  const estimated = current <= 15 ? 1.5 : current <= 21 ? 2.5 : current <= 28 ? 4 : current <= 36 ? 6 : current <= 50 ? 10 : current <= 68 ? 16 : current <= 89 ? 25 : 35;
  return commercialSections.find((item) => item >= estimated) ?? estimated;
}

function suggestBreaker(current: number, margin = 1.15): number {
  return commercialBreakers.find((item) => item >= current * margin) ?? Math.ceil(current * margin);
}
...
const rules: Rule[] = [
  ...
  { mode: 'dr-dps-checklist', module: 'eletricaResidencial', label: 'Checklist DR/DPS', description: 'Assistente textual de proteção.', plan: 'free', fields: [], options: [{ key: 'wetArea', label: 'Área molhada?', options: [{ value: 'yes', label: 'Sim' }, { value: 'no', label: 'Não' }] }, { key: 'outdoorOutlet', label: 'Tomada externa?', options: [{ value: 'yes', label: 'Sim' }, { value: 'no', label: 'Não' }] }, { key: 'sensitive', label: 'Equipamento sensível?', options: [{ value: 'yes', label: 'Sim' }, { value: 'no', label: 'Não' }] }], compute: ({ opt }) => { const recommendations = ['Conferir aterramento e separação de condutores.']; if (opt('wetArea') === 'yes' || opt('outdoorOutlet') === 'yes') recommendations.push('Avaliar DR para proteção adicional em áreas de maior risco.'); if (opt('sensitive') === 'yes') recommendations.push('Avaliar DPS e proteção contra surtos para equipamentos sensíveis.'); return result('Checklist DR/DPS gerado', [{ label: 'Itens', value: `${recommendations.length}` }], recommendations, 'Checklist orientativo para relatório. Projeto e adequação devem ser validados por profissional habilitado.', ['Assistente textual, sem fórmula numérica.'], { itemType: 'diagnostic', shouldGenerateBudgetItem: false }); } },
  { mode: 'sale-price-margin', module: 'financeiroAvancado', label: 'Preço por margem real', description: 'Preço de venda com margem sobre venda.', plan: 'free', fields: [{ key: 'cost', label: 'Custo', suffix: 'R$' }, { key: 'margin', label: 'Margem desejada', suffix: '%' }, { key: 'tax', label: 'Taxas/impostos', suffix: '%' }, { key: 'discount', label: 'Desconto', suffix: '%' }], compute: ({ n }) => { const price = n('cost', 'custo') / (1 - n('margin', 'margem') / 100); const withTax = price * (1 + n('tax', 'taxas') / 100); const final = withTax / (1 - n('discount', 'desconto') / 100); const profit = price - n('cost', 'custo'); return result(`Preço sugerido: ${money(final)}`, [{ label: 'Preço mínimo', value: money(price) }, { label: 'Preço sugerido', value: money(final) }, { label: 'Lucro', value: money(profit) }], [`Preço sugerido: ${money(final)}`], 'Margem real é calculada sobre o preço de venda, diferente de markup sobre custo.', ['Preço = custo ÷ (1 - margem)', 'Preço sugerido considera taxas e desconto planejado.']); } },
  { mode: 'sale-price-markup', module: 'financeiroAvancado', label: 'Preço por markup', description: 'Preço final com markup sobre custo.', plan: 'free', fields: [{ key: 'cost', label: 'Custo', suffix: 'R$' }, { key: 'markup', label: 'Markup', suffix: '%' }, { key: 'tax', label: 'Taxas', suffix: '%' }], compute: ({ n }) => { const price = n('cost', 'custo') * (1 + n('markup', 'markup') / 100) * (1 + n('tax', 'taxas') / 100); return result(`Preço final: ${money(price)}`, [{ label: 'Preço', value: money(price) }, { label: 'Lucro bruto', value: money(price - n('cost', 'custo')) }], [`Preço: ${money(price)}`], 'Markup é aplicado sobre o custo. A margem real resultante será menor que o percentual de markup.', ['Preço = custo × (1 + markup) × (1 + taxas).']); } },
...
const constructionRules: Rule[] = [
  ...
  { mode: 'baseboard', module: 'construcaoAvancada', label: 'Rodapé', description: 'Peças por perímetro.', plan: 'free', fields: [{ key: 'perimeter', label: 'Perímetro', suffix: 'm' }, { key: 'pieceLength', label: 'Tamanho peça', suffix: 'm' }, { key: 'loss', label: 'Perda', suffix: '%' }], compute: ({ n }) => { const total = n('perimeter', 'perímetro') * (1 + n('loss', 'perda') / 100); return result(`Rodapé: ${Math.ceil(total / n('pieceLength', 'peça'))} peças`, [{ label: 'Metros', value: `${round(total)} m` }], [`Metros: ${round(total)} m`], 'Desconte portas se necessário.', ['Peças = perímetro com perda ÷ tamanho da peça.']); } },
...
const converterRules: Rule[] = [
  { mode: 'awg-mm2', module: 'conversoresAvancados', label: 'mm² ↔ AWG', description: 'AWG próximo por seção.', plan: 'free', fields: [{ key: 'value', label: 'Seção', suffix: 'mm²' }], compute: ({ n }) => { const value = n('value', 'seção'); const nearest = awgTable.reduce((best, item) => Math.abs(item.mm2 - value) < Math.abs(best.mm2 - value) ? item : best, awgTable[0]); return result(`AWG próximo: ${nearest.awg}`, [{ label: 'AWG', value: nearest.awg }, { label: 'Tabela', value: `${nearest.mm2} mm²` }], [`AWG: ${nearest.awg}`], 'Conversão aproximada por tabela comum.', ['Busca seção AWG mais próxima.']); } },
  ...
  { mode: 'fraction-inch', module: 'conversoresAvancados', label: 'Fração de polegada', description: 'Decimal e mm aproximado.', plan: 'free', fields: [{ key: 'value', label: 'Polegada decimal', suffix: 'pol' }], compute: ({ n }) => { const sixteenths = Math.round(n('value', 'pol') * 16); return result(`${sixteenths}/16 pol`, [{ label: 'Fração', value: `${sixteenths}/16` }, { label: 'mm', value: `${round(n('value', 'pol') * 25.4)}` }], ['Fração aproximada'], 'Arredonda para dezesseis avos.', ['Fração = polegadas × 16.']); } },
  { mode: 'kgf-bar', module: 'conversoresAvancados', label: 'kgf/cm² ↔ bar', description: 'Pressão técnica.', plan: 'free', fields: [{ key: 'value', label: 'kgf/cm²' }], compute: ({ n }) => result(`${round(n('value', 'kgf') * 0.980665)} bar`, [{ label: 'bar', value: `${round(n('value', 'kgf') * 0.980665)}` }], ['kgf/cm² para bar'], 'Conversão aproximada.', ['bar = kgf/cm² × 0,980665.']) },
  ...
  { mode: 'kwh-money', module: 'conversoresAvancados', label: 'kWh ↔ R$', description: 'Custo por tarifa.', plan: 'free', fields: [{ key: 'value', label: 'Energia', suffix: 'kWh' }, { key: 'tariff', label: 'Tarifa', suffix: 'R$/kWh' }], compute: ({ n }) => result(`Custo: ${money(n('value', 'kWh') * n('tariff', 'tarifa'))}`, [{ label: 'Custo', value: money(n('value', 'kWh') * n('tariff', 'tarifa')) }], ['kWh para R$'], 'Use tarifa local com impostos quando possível.', ['Custo = kWh × tarifa.']) },
];
      ...
      {activeRule && (
        <div className="general-calculator-overlay" role="dialog" aria-modal="true" aria-label={activeRule.label}>
          ...
          <section className="general-overlay-panel">
            ...
            <small className="general-technical-note">Cálculo preliminar. Validar condições reais, normas aplicáveis, fabricante e medições antes de executar.</small>
          </section>
        </div>
      )}
    </div>
