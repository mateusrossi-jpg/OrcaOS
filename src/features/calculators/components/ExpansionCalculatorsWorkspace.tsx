...
interface ComputeContext {
  ...
  nn: (key: string, label: string) => number;
...
const rules: Rule[] = [
  ...
  { mode: 'sale-price-margin', module: 'financeiroAvancado', label: 'Preço por margem real', description: 'Preço de venda com margem sobre venda, taxas e desconto planejado.', plan: 'pro', fields: [{ key: 'cost', label: 'Custo', suffix: 'R$' }, { key: 'margin', label: 'Margem desejada', suffix: '%' }, { key: 'tax', label: 'Taxas/impostos', suffix: '%' }, { key: 'discount', label: 'Desconto', suffix: '%' }], compute: ({ n, nn }) => { const price = calculateSalePriceByTargetMargin({ cost: n('cost', 'custo'), marginPercent: n('margin', 'margem'), taxPercent: nn('tax', 'taxas'), plannedDiscountPercent: nn('discount', 'desconto') }); return result(`Preço sugerido: ${formatCurrency(price.suggestedPrice)}`, [{ label: 'Preço mínimo', value: formatCurrency(price.minimumPrice) }, { label: 'Preço sugerido', value: formatCurrency(price.suggestedPrice) }, { label: 'Lucro', value: formatCurrency(price.profit) }], [`Preço sugerido: ${formatCurrency(price.suggestedPrice)}`], 'Margem real é calculada sobre o preço de venda, diferente de markup sobre custo.', ['Preço = custo ÷ (1 - margem)', 'Preço sugerido considera taxas e desconto planejado.']); } },
  { mode: 'sale-price-markup', module: 'financeiroAvancado', label: 'Preço por markup', description: 'Preço final com markup sobre custo e taxas.', plan: 'pro', fields: [{ key: 'cost', label: 'Custo', suffix: 'R$' }, { key: 'markup', label: 'Markup', suffix: '%' }, { key: 'tax', label: 'Taxas', suffix: '%' }], compute: ({ n, nn }) => { const price = calculateSalePriceByMarkup({ cost: n('cost', 'custo'), markupPercent: n('markup', 'markup'), taxPercent: nn('tax', 'taxas') }); return result(`Preço final: ${formatCurrency(price.finalPrice)}`, [{ label: 'Preço', value: formatCurrency(price.finalPrice) }, { label: 'Lucro bruto', value: formatCurrency(price.grossProfit) }], [`Preço: ${formatCurrency(price.finalPrice)}`], 'Markup é aplicado sobre o custo. A margem real resultante será menor que o percentual de markup.', ['Preço = custo × (1 + markup) × (1 + taxas).']); } },
  { mode: 'max-discount', module: 'financeiroAvancado', label: 'Desconto máximo', description: 'Desconto sem cair abaixo da margem mínima.', plan: 'pro', fields: [{ key: 'price', label: 'Preço atual', suffix: 'R$' }, { key: 'cost', label: 'Custo', suffix: 'R$' }, { key: 'minMargin', label: 'Margem mínima', suffix: '%' }], compute: ({ n }) => { const minPrice = n('cost', 'custo') / (1 - n('minMargin', 'margem') / 100); const discount = Math.max(0, (n('price', 'preço') - minPrice) / n('price', 'preço') * 100); const status = n('price', 'preço') < minPrice ? 'Abaixo da margem' : 'Pode negociar'; return result(`Desconto máximo: ${round(discount)}%`, [{ label: 'Preço mínimo', value: formatCurrency(minPrice) }, { label: 'Desconto', value: `${round(discount)}%` }, { label: 'Status', value: status }], [`Preço mínimo: ${formatCurrency(minPrice)}`], 'Use para negociação sem destruir a margem mínima.', ['Preço mínimo = custo ÷ (1 - margem mínima)', 'Desconto máximo = (preço atual - preço mínimo) ÷ preço atual.']); } },
  { mode: 'installment-fee', module: 'financeiroAvancado', label: 'Parcelamento com taxa', description: 'Parcela e custo financeiro.', plan: 'pro', fields: [{ key: 'price', label: 'Preço à vista', suffix: 'R$' }, { key: 'monthlyFee', label: 'Taxa mensal', suffix: '%' }, { key: 'installments', label: 'Parcelas' }], options: [{ key: 'feePayer', label: 'Quem absorve?', options: [{ value: 'client', label: 'Cliente' }, { value: 'professional', label: 'Profissional' }] }], compute: ({ n, opt }) => { const fee = n('monthlyFee', 'taxa') / 100 * n('installments', 'parcelas'); const final = opt('feePayer') === 'client' ? n('price', 'preço') * (1 + fee) : n('price', 'preço'); const cost = n('price', 'preço') * fee; return result(`${round(n('installments', 'parcelas'))}x de ${formatCurrency(final / n('installments', 'parcelas'))}`, [{ label: 'Preço final', value: formatCurrency(final) }, { label: 'Custo financeiro', value: formatCurrency(cost) }], [`Preço final: ${formatCurrency(final)}`], 'Taxa simplificada. Confira taxa real da maquininha/plataforma.', ['Taxa total simplificada = taxa mensal × parcelas.']); } },
  { mode: 'recommended-upfront', module: 'financeiroAvancado', label: 'Entrada recomendada', description: 'Sinal por material e risco.', plan: 'pro', fields: [{ key: 'price', label: 'Valor total', suffix: 'R$' }, { key: 'materialCost', label: 'Custo material', suffix: 'R$' }, { key: 'margin', label: 'Entrada desejada', suffix: '%' }, { key: 'risk', label: 'Risco', suffix: '1-3' }], compute: ({ n }) => { const base = Math.max(n('price', 'total') * n('margin', 'entrada') / 100, n('materialCost', 'material')); const recommended = base * (1 + (n('risk', 'risco') - 1) * 0.1); return result(`Sinal recomendado: ${formatCurrency(recommended)}`, [{ label: 'Sinal', value: formatCurrency(recommended) }, { label: 'Saldo', value: formatCurrency(n('price', 'total') - recommended) }], [`Sinal: ${formatCurrency(recommended)}`], 'Use para cobrir material, agenda e risco do serviço.', ['Sinal = maior valor entre percentual do total e custo de material, ajustado pelo risco.']); } },
  { mode: 'real-hourly-rate', module: 'financeiroAvancado', label: 'Hora técnica real', description: 'Hora por renda, custos fixos e margem.', plan: 'pro', fields: [{ key: 'desiredIncome', label: 'Renda desejada', suffix: 'R$/mês' }, { key: 'fixedCost', label: 'Custos fixos', suffix: 'R$/mês' }, { key: 'workDays', label: 'Dias úteis' }, { key: 'productiveHours', label: 'Horas produtivas/dia' }, { key: 'margin', label: 'Margem', suffix: '%' }], compute: ({ n }) => { const monthly = n('desiredIncome', 'renda') + n('fixedCost', 'custos'); const min = monthly / (n('workDays', 'dias') * n('productiveHours', 'horas')); const rec = min * (1 + n('margin', 'margem') / 100); return result(`Hora recomendada: ${formatCurrency(rec)}`, [{ label: 'Hora mínima', value: formatCurrency(min) }, { label: 'Hora recomendada', value: formatCurrency(rec) }], [`Hora recomendada: ${formatCurrency(rec)}`], 'Ajuda a evitar cobrar só pelo tempo visível e esquecer custos fixos.', ['Hora mínima = (renda + custos fixos) ÷ horas produtivas mensais.']); } },
  { mode: 'daily-rate-advanced', module: 'financeiroAvancado', label: 'Diária técnica', description: 'Diária mínima e recomendada.', plan: 'pro', fields: [{ key: 'hourlyRate', label: 'Hora técnica', suffix: 'R$/h' }, { key: 'hours', label: 'Horas/diária' }, { key: 'cost', label: 'Deslocamento', suffix: 'R$' }, { key: 'helperCost', label: 'Ajudante', suffix: 'R$' }, { key: 'margin', label: 'Margem', suffix: '%' }], compute: ({ n }) => { const min = n('hourlyRate', 'hora') * n('hours', 'horas') + n('cost', 'deslocamento') + n('helperCost', 'ajudante'); const rec = min * (1 + n('margin', 'margem') / 100); return result(`Diária recomendada: ${formatCurrency(rec)}`, [{ label: 'Mínima', value: formatCurrency(min) }, { label: 'Recomendada', value: formatCurrency(rec) }], [`Diária: ${formatCurrency(rec)}`], 'Defina o que está incluso na diária antes de enviar a proposta.', ['Diária mínima = hora × horas + deslocamento + ajudante.']); } },
  { mode: 'travel-cost', module: 'financeiroAvancado', label: 'Deslocamento', description: 'Combustível, tempo e visita.', plan: 'pro', fields: [{ key: 'km', label: 'Km ida/volta' }, { key: 'consumption', label: 'Consumo', suffix: 'km/L' }, { key: 'fuelPrice', label: 'Combustível', suffix: 'R$/L' }, { key: 'travelHours', label: 'Tempo', suffix: 'h' }, { key: 'hourlyRate', label: 'Hora', suffix: 'R$/h' }], compute: ({ n }) => { const fuel = n('km', 'km') / n('consumption', 'consumo') * n('fuelPrice', 'combustível'); const time = n('travelHours', 'tempo') * n('hourlyRate', 'hora'); return result(`Visita sugerida: ${formatCurrency(fuel + time)}`, [{ label: 'Combustível', value: formatCurrency(fuel) }, { label: 'Tempo', value: formatCurrency(time) }, { label: 'Total', value: formatCurrency(fuel + time) }], [`Deslocamento: ${formatCurrency(fuel + time)}`], 'Inclua pedágio/estacionamento quando houver.', ['Combustível = km ÷ consumo × preço', 'Tempo = horas × valor/hora.']); } },
  { mode: 'price-bands', module: 'financeiroAvancado', label: 'Preço mínimo/ideal/premium', description: 'Três faixas de preço por margem.', plan: 'pro', fields: [{ key: 'cost', label: 'Custo total', suffix: 'R$' }, { key: 'minMargin', label: 'Margem mínima', suffix: '%' }, { key: 'idealMargin', label: 'Margem ideal', suffix: '%' }, { key: 'premiumMargin', label: 'Margem premium', suffix: '%' }], compute: ({ n }) => { const price = (m: number) => n('cost', 'custo') / (1 - m / 100); return result('Faixas comerciais calculadas', [{ label: 'Mínimo', value: formatCurrency(price(n('minMargin', 'mínima'))) }, { label: 'Ideal', value: formatCurrency(price(n('idealMargin', 'ideal'))) }, { label: 'Premium', value: formatCurrency(price(n('premiumMargin', 'premium'))) }], ['Faixas de preço geradas'], 'Use mínimo para negociação limitada, ideal para proposta padrão e premium para urgência/garantia/complexidade.', ['Preço = custo ÷ (1 - margem).']); } },
  { mode: 'tax-fee-gross-up', module: 'financeiroAvancado', label: 'Receber líquido desejado', description: 'Preço bruto para sobrar um valor líquido após taxas.', plan: 'pro', fields: [{ key: 'desiredNet', label: 'Quero receber líquido', suffix: 'R$' }, { key: 'tax', label: 'Imposto/taxa', suffix: '%' }, { key: 'cardFee', label: 'Taxa cartão/plataforma', suffix: '%' }], compute: ({ n, nn }) => { const net = n('desiredNet', 'valor líquido'); const totalRate = (nn('tax', 'taxas') + nn('cardFee', 'taxa de cartão')) / 100; const gross = net / (1 - totalRate); const fees = gross - net; return result(`Cobrar: ${formatCurrency(gross)}`, [{ label: 'Valor bruto', value: formatCurrency(gross) }, { label: 'Taxas estimadas', value: formatCurrency(fees) }, { label: 'Líquido', value: formatCurrency(net) }], [`Cobrar ${formatCurrency(gross)} para receber ${formatCurrency(net)} líquido`], 'Use quando você sabe quanto precisa sobrar depois de taxa, imposto ou plataforma.', ['Preço bruto = líquido desejado ÷ (1 - taxa total).']); } },
  { mode: 'card-fee-net', module: 'financeiroAvancado', label: 'Quanto sobra no cartão', description: 'Valor líquido após taxa de cartão e imposto.', plan: 'pro', fields: [{ key: 'price', label: 'Valor cobrado', suffix: 'R$' }, { key: 'cardFee', label: 'Taxa cartão', suffix: '%' }, { key: 'tax', label: 'Imposto/taxa', suffix: '%' }], compute: ({ n, nn }) => { const price = n('price', 'valor'); const card = price * nn('cardFee', 'cartão') / 100; const tax = price * nn('tax', 'imposto') / 100; const net = price - card - tax; return result(`Líquido estimado: ${formatCurrency(net)}`, [{ label: 'Cartão', value: formatCurrency(card) }, { label: 'Imposto/taxa', value: formatCurrency(tax) }, { label: 'Sobra', value: formatCurrency(net) }], [`Líquido: ${formatCurrency(net)}`], 'Útil para decidir se absorve taxa ou repassa ao cliente.', ['Líquido = valor cobrado - taxa cartão - imposto/taxa.']); } },
  { mode: 'cash-vs-installment', module: 'financeiroAvancado', label: 'À vista vs parcelado', description: 'Compare desconto à vista com parcelamento.', plan: 'pro', fields: [{ key: 'price', label: 'Preço base', suffix: 'R$' }, { key: 'discount', label: 'Desconto à vista', suffix: '%' }, { key: 'cardFee', label: 'Taxa do parcelado', suffix: '%' }, { key: 'installments', label: 'Parcelas' }], compute: ({ n, nn }) => { const price = n('price', 'preço'); const cash = price * (1 - nn('discount', 'desconto') / 100); const installmentFee = price * nn('cardFee', 'taxa') / 100; const installmentNet = price - installmentFee; return result(`À vista: ${formatCurrency(cash)} · Parcelado líquido: ${formatCurrency(installmentNet)}`, [{ label: 'À vista', value: formatCurrency(cash) }, { label: 'Parcelado líquido', value: formatCurrency(installmentNet) }, { label: 'Parcela', value: formatCurrency(price / n('installments', 'parcelas')) }], ['Comparação comercial gerada'], 'Compare o que realmente sobra, não apenas o valor anunciado ao cliente.', ['À vista = preço × (1 - desconto)', 'Líquido parcelado = preço - taxa.']); } },
  { mode: 'break-even-services', module: 'financeiroAvancado', label: 'Serviços para empatar', description: 'Quantos serviços pagam custos fixos e renda alvo.', plan: 'pro', fields: [{ key: 'fixedCost', label: 'Custos fixos mensais', suffix: 'R$' }, { key: 'desiredIncome', label: 'Renda desejada', suffix: 'R$' }, { key: 'averageProfit', label: 'Lucro médio por serviço', suffix: 'R$' }], compute: ({ n }) => { const target = n('fixedCost', 'custos fixos') + n('desiredIncome', 'renda'); const services = Math.ceil(target / n('averageProfit', 'lucro médio')); return result(`${services} serviço(s) para atingir a meta`, [{ label: 'Meta mensal', value: formatCurrency(target) }, { label: 'Lucro médio', value: formatCurrency(n('averageProfit', 'lucro médio')) }, { label: 'Serviços', value: `${services}` }], [`Meta mensal: ${formatCurrency(target)}`], 'Ajuda a transformar custo fixo e renda desejada em volume mínimo de serviços.', ['Serviços = (custos fixos + renda desejada) ÷ lucro médio por serviço.']); } },
  { mode: 'monthly-goal', module: 'financeiroAvancado', label: 'Meta mensal por ticket', description: 'Faturamento e atendimentos necessários no mês.', plan: 'pro', fields: [{ key: 'desiredIncome', label: 'Meta de renda/lucro', suffix: 'R$' }, { key: 'fixedCost', label: 'Custos fixos', suffix: 'R$' }, { key: 'averageTicket', label: 'Ticket médio', suffix: 'R$' }, { key: 'margin', label: 'Margem líquida média', suffix: '%' }], compute: ({ n }) => { const neededProfit = n('desiredIncome', 'meta') + n('fixedCost', 'custos'); const profitPerTicket = n('averageTicket', 'ticket') * n('margin', 'margem') / 100; const jobs = Math.ceil(neededProfit / profitPerTicket); const revenue = jobs * n('averageTicket', 'ticket'); return result(`${jobs} atendimento(s) no mês`, [{ label: 'Faturamento alvo', value: formatCurrency(revenue) }, { label: 'Lucro/meta', value: formatCurrency(neededProfit) }, { label: 'Atendimentos', value: `${jobs}` }], [`Faturamento alvo: ${formatCurrency(revenue)}`], 'Use para saber se a agenda e o preço médio sustentam sua meta.', ['Atendimentos = meta de lucro ÷ (ticket médio × margem líquida).']); } },
  { mode: 'material-resale-price', module: 'financeiroAvancado', label: 'Revenda de material', description: 'Preço de material com margem, taxa e desconto.', plan: 'pro', fields: [{ key: 'materialCost', label: 'Custo do material', suffix: 'R$' }, { key: 'margin', label: 'Margem desejada', suffix: '%' }, { key: 'tax', label: 'Taxa/imposto', suffix: '%' }, { key: 'discount', label: 'Desconto possível', suffix: '%' }], compute: ({ n, nn }) => { const price = calculateSalePriceByTargetMargin({ cost: n('materialCost', 'material'), marginPercent: n('margin', 'margem'), taxPercent: nn('tax', 'taxa'), plannedDiscountPercent: nn('discount', 'desconto') }); return result(`Material por: ${formatCurrency(price.suggestedPrice)}`, [{ label: 'Custo', value: formatCurrency(n('materialCost', 'material')) }, { label: 'Venda', value: formatCurrency(price.suggestedPrice) }, { label: 'Lucro', value: formatCurrency(price.profit) }], [`Preço de venda do material: ${formatCurrency(price.suggestedPrice)}`], 'Útil quando o profissional compra, garante ou entrega material ao cliente.', ['Preço = custo do material ÷ (1 - margem), com taxa e desconto planejado.']); } },
  { mode: 'warranty-reserve', module: 'financeiroAvancado', label: 'Reserva para garantia', description: 'Separe parte do orçamento para retorno, garantia ou retrabalho.', plan: 'pro', fields: [{ key: 'price', label: 'Valor do serviço', suffix: 'R$' }, { key: 'warrantyPercent', label: 'Reserva', suffix: '%' }], compute: ({ n }) => { const reserve = n('price', 'valor') * n('warrantyPercent', 'reserva') / 100; return result(`Reserva sugerida: ${formatCurrency(reserve)}`, [{ label: 'Reserva', value: formatCurrency(reserve) }, { label: 'Disponível', value: formatCurrency(n('price', 'valor') - reserve) }], [`Reserva: ${formatCurrency(reserve)}`], 'Ajuda a não tratar garantia/retrabalho como surpresa financeira.', ['Reserva = valor do serviço × percentual de reserva.']); } },
  { mode: 'emergency-price', module: 'financeiroAvancado', label: 'Preço de urgência', description: 'Acréscimo para atendimento urgente, noite, fim de semana ou risco.', plan: 'pro', fields: [{ key: 'price', label: 'Preço normal', suffix: 'R$' }, { key: 'emergencyPercent', label: 'Acréscimo de urgência', suffix: '%' }, { key: 'travelCost', label: 'Deslocamento extra', suffix: 'R$' }], compute: ({ n, nn }) => { const surcharge = n('price', 'preço') * n('emergencyPercent', 'urgência') / 100; const final = n('price', 'preço') + surcharge + nn('travelCost', 'deslocamento'); return result(`Preço de urgência: ${formatCurrency(final)}`, [{ label: 'Acréscimo', value: formatCurrency(surcharge) }, { label: 'Deslocamento', value: formatCurrency(nn('travelCost', 'deslocamento')) }, { label: 'Total', value: formatCurrency(final) }], [`Preço urgente: ${formatCurrency(final)}`], 'Use para atendimento fora do padrão sem misturar urgência com orçamento comum.', ['Total = preço normal + acréscimo de urgência + deslocamento extra.']); } },
];
...
const converterRules: Rule[] = [
  ...
  { mode: 'kwh-money', module: 'conversoresAvancados', label: 'kWh ↔ R$', description: 'Custo por tarifa.', plan: 'pro', fields: [{ key: 'value', label: 'Energia', suffix: 'kWh' }, { key: 'tariff', label: 'Tarifa', suffix: 'R$/kWh' }], compute: ({ n }) => result(`Custo: ${formatCurrency(n('value', 'kWh') * n('tariff', 'tarifa'))}`, [{ label: 'Custo', value: formatCurrency(n('value', 'kWh') * n('tariff', 'tarifa')) }], ['kWh para R$'], 'Use tarifa local com impostos quando possível.', ['Custo = kWh × tarifa.']) },
];

const allRules = [...rules, ...constructionRules, ...hydraulicRules, ...converterRules];

export function getExpansionCalculatorPlanStats(selectedModule: CalculatorModule) {
  const moduleRules = allRules.filter((rule) => rule.module === selectedModule);
  return {
    total: moduleRules.length,
    free: moduleRules.filter((rule) => rule.plan === 'free').length,
    pro: moduleRules.filter((rule) => rule.plan === 'pro').length,
  };
}

function NumberField({ field, value, onChange }: { field: Field; value: string; onChange: (value: string) => void }) {
  return <label className="general-form-field"><span>{field.label}</span><div><input type="number" inputMode="decimal" min="0" step={field.step ?? 0.01} value={value} onFocus={handleNumericInputFocus} onChange={(event) => onChange(event.target.value)} />{field.suffix && <small className="technical-unit">{field.suffix}</small>}</div></label>;
}
  ...
  const computed = useMemo<ExpansionResult>(() => {
    ...
    try {
      return activeRule.compute({
        ...
        nn: (key, label) => nonNegative(parseNumber(values[key] ?? ''), label),
        opt: (key) => options[key] ?? '',
      });
    } catch (error) {
