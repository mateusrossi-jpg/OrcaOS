...
const calculationModules: ModuleCardData[] = [
  { id: 'fundamentosGerais', title: 'Fundamentos gerais', description: 'Regra de três, porcentagem, áreas, volumes e custo por m²', icon: '∑', tone: 'green', count: '7 cálculos livres', available: true, plan: 'free', calculatorModule: 'fundamentosGerais' },
  ...
  { id: 'construcaoCivil', title: 'Construção civil', description: 'Medições, concreto, blocos, alvenaria, piso e revestimento', icon: '▧', tone: 'gray', count: '5 cálculos livres', available: true, plan: 'free', calculatorModule: 'obras' },
  { id: 'pintura', title: 'Pintura e acabamento', description: 'Área a pintar, litros de tinta, demãos e orçamento por m²', icon: '▨', tone: 'green', count: '3 cálculos livres', available: true, plan: 'free', calculatorModule: 'pintura' },
...
const storePackages = [
  { title: 'Fundamentos gerais grátis', description: 'Regra de três, porcentagem, acréscimo, desconto, área, volume e custo por m².', price: 'R$ 0', action: 'Já incluso', icon: '∑' },
  { title: 'Fundamentos elétricos grátis', description: 'Lei de Ohm, corrente, potência, resistores, W/VA/A e consumo liberados sem assinatura.', price: 'R$ 0', action: 'Já incluso', icon: 'ϟ' },
  ...
  { title: 'Pacote Construção Civil Pro', description: 'Concreto, alvenaria, revestimento, composição e levantamentos por ambiente.', price: 'R$ 12,90', action: 'Detalhes', icon: '▧' },
  ...
  { title: 'Pacote Orçamentos Pro', description: 'Modelos avançados, identidade profissional, PDF e recursos comerciais.', price: 'R$ 12,90', action: 'Detalhes', icon: '▣' },
];
    ...
    <section className="app-screen orca-dashboard-screen">
      <div className="orca-dashboard-hero">
        <div className="orca-dashboard-copy">
          ...
          <p>Agora os cálculos seguem a estrutura: núcleo transversal, família elétrica/eletrotécnica, construção civil, pintura, hidráulica, refrigeração e orçamento.</p>
        </div>
      ...
      <div className="orca-quick-actions">
        <button type="button" onClick={() => openModule(calculationModules[0])}><span className="app-icon tone-green">∑</span><strong>Fundamentos gerais</strong><small>7 cálculos livres</small></button>
      ...
      <div className="orca-home-columns">
        <section className="orca-panel-card">
          ...
          <div className="orca-compact-module-list">
            {calculationModules.slice(0, 7).map((module) => (
...
function CalculationsScreen({ selectedModule, openModule, goTo, onCaptureCalculation }: { selectedModule: ModuleCardData | null; openModule: (module: ModuleCardData | null) => void; goTo: (tab: AppTab) => void; onCaptureCalculation: (capture: CalculationCapture) => void }) {
  if (selectedModule) {
    ...
    return (
      <section className="app-screen">
        ...
        {module && module !== 'fundamentosGerais' && !isGeneralCalculatorModule(module) && <ElectricalCalculatorWorkspace selectedModule={module} userPlan={userPlan} onUpgradeRequest={() => goTo('store')} onCaptureCalculation={onCaptureCalculation} />}
  ...
  return <section className="app-screen calculations-overview-screen"><header className="screen-header"><span className="orca-kicker">Famílias e módulos</span><h1>Cálculos</h1><p>Escolha o módulo. O OrçaOS agora separa núcleo transversal, elétrica/eletrotécnica, construção civil, pintura, refrigeração e orçamento técnico.</p></header><div className="module-list-app">{calculationModules.map((module) => <ModuleCard key={module.id} module={module} compact onOpen={() => openModule(module)} />)}</div></section>;
}
