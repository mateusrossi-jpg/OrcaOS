  ...
  const renderAttention = () => {
    ...
    return (
      <div className="flex flex-col gap-1.5">
        ...
        <div className="bg-[#FF453A]/10 border border-[#FF453A]/20 rounded-[16px] p-3 flex flex-col gap-2">
          {alerts.map((a, i) => (
    ...
    <div className="flex flex-col gap-1.5">
      ...
      {nextActionOS ? (
         <div 
           ...
           className="bg-[#0A84FF]/10 border border-[#0A84FF]/20 rounded-[16px] p-3.5 flex flex-col gap-2.5 relative overflow-hidden group cursor-pointer active:scale-[0.98] transition-all"
         >
    ...
    <div className="flex flex-col gap-1.5">
      ...
      {topOpportunity ? (
        <div 
          ...
          className="bg-[var(--accent-gold)]/5 border border-[var(--accent-gold)]/10 rounded-[16px] p-3.5 relative overflow-hidden group flex justify-between items-center gap-3 cursor-pointer active:scale-[0.98] transition-all"
        >
          <div className="flex flex-col min-w-0">
            <h3 className="text-sm font-black text-white tracking-tight leading-snug truncate">
  ...
  const renderMoney = () => (
    <div className="flex flex-col gap-1.5">
      ...
      <div 
        ...
        className="bg-[#248A3D]/5 border border-[#248A3D]/10 p-3.5 rounded-[16px] flex justify-between items-center group active:scale-[0.98] transition-all cursor-pointer"
      >
        ...
        <div className="flex flex-col gap-1.5">
           ...
           <div className="grid grid-cols-2 gap-2">
             <GridTool 
               ...
               title="ORÇAMENTOS" 
               sub={stats.followUps > 0 ? \`\${stats.followUps} em aberto\` : "Nenhum em aberto"}
               onClick={() => onNavigate('new-budget')} 
             />
             <GridTool 
               icon={<Users size={14} className="text-[#0A84FF]" />} 
               title="CLIENTES" 
               sub="Base de contatos"
               onClick={() => onNavigate('relationships')} 
             />
             <GridTool 
               icon={<DollarSign size={14} className="text-[#30D158]" />} 
               title="FINANCEIRO" 
               sub={stats.toCollect > 0 ? "Com pendências" : "Nenhuma pendência"}
               onClick={() => onNavigate('revenue')} 
             />
             <GridTool 
               icon={<Settings size={14} className="text-[#AF52DE]" />} 
               title="PLANOS" 
               sub="Serviços recorrentes"
               onClick={() => onNavigate('relationships')} 
             />
           </div>
        </div>
...
function SearchRow({ title, sub, onClick }: any) {
  return (
    <div 
      onClick={onClick}
      className="bg-white/[0.02] border border-white/[0.04] p-2.5 rounded-[10px] flex items-center justify-between gap-2 cursor-pointer active:scale-[0.98] transition-all"
    >
      <div className="flex flex-col min-w-0">
